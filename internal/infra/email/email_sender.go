...
func (m *SmtpMailer) SendEmail(name string, emailAddr string, inviteCode string, t usecase.EmailType) error {
	subject, body := getEmailContent(t, name, inviteCode)

	c := email.EmailConfig{
		FromEmail:    config.Config().Mailer.FromEmail,
		FromPassword: config.Config().Mailer.FromPassword,
